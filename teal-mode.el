...
;; lua-mode. The following is copied from lua-mode's commentary:
...
(defvar teal-mode-menu (make-sparse-keymap "Lua")
...
(defvar teal-mode-map
  (let ((result-map (make-sparse-keymap)))
    ...
    (define-key result-map [menu-bar teal-mode] (cons "Lua" teal-mode-menu))
...
;;;###autoload
(define-derived-mode teal-mode prog-mode "Lua"
  "Major mode for editing Lua code."
...
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.lua\\'" . teal-mode))

;;;###autoload
(add-to-list 'interpreter-mode-alist '("lua" . teal-mode))
...
(defun lua-prefix-key-update-bindings ()
  (let (old-cons)
    (if (eq lua-prefix-mode-map (keymap-parent teal-mode-map))
        ;; if prefix-map is a parent, delete the parent
        (set-keymap-parent teal-mode-map nil)
      ;; otherwise, look for it among children
      (if (setq old-cons (rassoc lua-prefix-mode-map teal-mode-map))
          (delq old-cons teal-mode-map)))

    (if (null lua-prefix-key)
        (set-keymap-parent teal-mode-map lua-prefix-mode-map)
      (define-key teal-mode-map (vector lua-prefix-key) lua-prefix-mode-map))))
...
(define-key teal-mode-menu [send-lua-region]
  '("Send Lua-Region" . lua-send-lua-region))
(define-key teal-mode-menu [set-lua-region-end]
  '("Set Lua-Region End" . lua-set-lua-region-end))
(define-key teal-mode-menu [set-lua-region-start]
